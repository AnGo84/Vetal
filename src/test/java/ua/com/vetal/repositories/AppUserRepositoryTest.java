...
class AppUserRepositoryTest {
    ...
    public void it_should_save_user() {
        // when
        AppUser newAppUser = AppUser.builder().userName("User2").enabled(true).encryptedPassword("second pass").build();
        newAppUser = appUserRepository.save(newAppUser);
        AppUser foundAppUser = appUserRepository.findByUserName(newAppUser.getUserName());

        // then
        assertNotNull(foundAppUser);
        assertNotNull(foundAppUser.getUserId());
        assertEquals(newAppUser.getUserName(), foundAppUser.getUserName());
        assertEquals(newAppUser.getEncryptedPassword(), foundAppUser.getEncryptedPassword());
        assertEquals(newAppUser.isEnabled(), foundAppUser.isEnabled());
    }
